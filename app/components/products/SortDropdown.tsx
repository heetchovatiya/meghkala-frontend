...
export function SortDropdown() {
 ...
 <Menu as="div" className="relative inline-block text-left w-full">
      <div>
        {/* ✅ REMOVED "Sort by:" text from the button for a cleaner look */}
          ...
          <div className="py-1">
            {sortOptions.map(option => (
              <Menu.Item key={option.value}>
                {({ active }) => (
                  <button onClick={() => handleSortChange(option.value)} className={`${ active ? 'bg-secondary-bg' : ''} group flex w-full items-center px-4 py-2 text-sm text-left`}>
                    {option.name}
                  </button>
                )}
              </Menu.Item>
